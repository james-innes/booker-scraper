...
class login_booker:
    ...
    def preprocessing(self):
        ...
        accNo = self.driver.find_element_by_id("OutsideHomePageControl_CustomerNumber")
        accNo.send_keys(loginInfo[0])
        accNo.send_keys(Keys.RETURN)
        time.sleep(0.5)

        self.driver.get("https://www.booker.co.uk/account/loginregister/UserLogin.aspx")
        email = self.driver.find_element_by_id("LoginControl_EmailSingle")
        email.send_keys(loginInfo[1])
        password = self.driver.find_element_by_id("LoginControl_PasswordSingle")
        password.send_keys(loginInfo[2])
        
        butLogin = self.driver.find_element_by_id("LoginControl_EnterEmailPasswordSubmit")
        butLogin.click()
        self.start_scraping()
    ...
    def link_to_json(self):
        ...
        imageSmallGuid = img[0].get_attribute('src').split('/')[-1].split('.')[0]
        ...
        json_format = {}
        prodInfo = prodInfo.text.split('\n')[:-2]  # remove 'Number of Trolley' and 'Add to Shopping list' options
        
        # converting product info to json
        json_format = dict([(i.split(':',1)[0], i.split(':',1)[1].strip()) for i in prodInfo])
        ...
        for i in productDetail:
            ...
            if len(i) < 3:
                ...
                json_format.update(dict([tuple(j.split(':',1)) for j in i[1:]]))
            elif "Nutrition" in i[0]:
                json_format[i[0]] = dict([tuple(j.rsplit(' ',1)) for j in i[1:]])
            elif "Country" in i[0]:
                json_format.update( dict([tuple(j.rsplit(' ',1)) for j in i[1:]]))
