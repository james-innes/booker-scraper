...
class ProductListSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for row in sqlite3.connect('stores.db').execute("SELECT * FROM sitemap").fetchall():
            ...
            yield Request(
                url=f'https://www.booker.co.uk/products/product-list?categoryName={row[1]}', cookies={'ASP.NET_SessionId': os.getenv('ASP_NET_SESSION'), '.ASPXAUTH': os.getenv('ASPXAUTH'), 'BookerMessage': 'WebsiteBulletinCheckedDate=15%2f03%2f2021+14%3a01%3a00'}, callback=self.parse_product_list, cb_kwargs=dict(sub_cat_code=row[1]))
    ...
    def parse_product_list(self, response, sub_cat_code):
        for row in response.css('.rowUnGrouped .product-model'):
            ...
            l.add_css('temperature',
                      '.product-number > img[alt$=" Product"]::attr(alt)')
        ...
        if next_page_url is not None:
            yield Request(next_page_url, cookies={'ASP.NET_SessionId': os.getenv('ASP_NET_SESSION'), '.ASPXAUTH': os.getenv('ASPXAUTH'), 'BookerMessage': 'WebsiteBulletinCheckedDate=15%2f03%2f2021+14%3a01%3a00'}, callback=self.parse_product_list, cb_kwargs=dict(sub_cat_code=sub_cat_code))
