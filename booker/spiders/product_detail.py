...
class ProductDetailSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for row in (
            sqlite3.connect("stores.db")
            .execute("SELECT * FROM product_list")
            .fetchall()
        ):
            yield Request(
                url=f"https://www.booker.co.uk/products/product%20detail?Code={row[1]}",
                cookies={
                    "ASP.NET_SessionId": os.getenv("ASP_NET_SESSION"),
                    ".ASPXAUTH": os.getenv("ASPXAUTH"),
                    "BookerMessage": "WebsiteBulletinCheckedDate=21%2f01%2f2021+18%3a47%3a01",
                },
                callback=self.parse_product_detail,
                cb_kwargs=dict(code=row[1]),
            )

    def parse_product_detail(self, response, code):
        description = "".join(
            response.css("#product-details-show-more :not([id^=show-less])").extract()
        )

        if response.css("#categories p").extract():
            catagories = (
                "<h4>Categories</h4>" + response.css("#categories p").extract()[0]
            )
        else:
            catagories = ""
        ...
        for card in response.css(".desplegabledesktop .product-cards .card"):
            header = "<h4>" + card.css(".card-header h4::text").extract()[0] + "</h4>"
