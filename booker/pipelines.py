    ...
    def process_item(self, item, spider):
        ...
        def clean_html(html):
            ...
            for comment in soup.findAll(
                text=lambda text: isinstance(text, Comment)): comment.extract()
        ...
        if spider.name == "product_list":
            ...
            if adapter.get('por'):
                adapter['por'] = clean_value(adapter.get('por')[0])
            if adapter.get('rrp'):
                adapter['rrp'] = clean_value(
                    adapter.get('rrp')[0]).replace(',', '')
            adapter['wsp_inc_vat'] = clean_value(
                adapter.get('wsp_inc_vat')[0]).replace(',', '')
            if adapter.get('case'):
                adapter['case'] = clean_value(adapter.get('case')[0])
            if adapter.get('temperature'):
                adapter['temperature'] = clean_value(
                    adapter.get('temperature')[0])
            else:
                adapter['temperature'] = "Ambient"
